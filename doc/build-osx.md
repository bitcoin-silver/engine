...
It is not necessary to build wallet functionality to run `namecoind` or  `bitcoinsilver-qt`.
...
2.  Build Bitcoinsilver Core:
    ...
    Configure and build the headless Bitcoinsilver Core binaries as well as the GUI (if Qt is found).
...
Bitcoinsilver Core should now be available at `./build/bin/namecoind`.
If you compiled support for the GUI, it should be available at `./build/bin/bitcoinsilver-qt`.

The first time you run `namecoind` or `bitcoinsilver-qt`, it will start downloading the blockchain.
...
```shell
./build/bin/namecoind -daemon      # Starts the bitcoinsilver daemon.
