...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    /* PolicyScriptChecks uses CheckInputScripts, while the bitcoinsilver-specific
       transaction checking is part of CheckTxInputs called here.  Thus we need
       to make sure all standard flags relevant for Bitcoinsilver checks are
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    //
    // Bitcoinsilver actually allows some scripts into the mempool that would
...
/* Temporary check that blocks are compatible with BDB's 10,000 lock limit.
   ...
   In addition, each Bitcoinsilver transaction could touch at most one name,
   so add them as well.  */
