...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitcoinsilver-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitcoinsilver-tx tool is used for creating and modifying transactions.\n\n"
                "bitcoinsilver-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded transaction, or with \"-create [commands]\" to create a hex-encoded transaction.\n"
                "\n"
                "Usage: bitcoinsilver-tx [options] <hex-tx> [commands]\n"
                "or:    bitcoinsilver-tx [options] -create [commands]\n"
                "\n";
