...
BOOST_AUTO_TEST_CASE(FeeRounder)
{
    ...
    /* Bitcoinsilver uses a different DEFAULT_MIN_RELAY_TX_FEE value than
       upstream Bitcoin.  Verify the expected rounding value for MAX_MONEY
       (which is used as fee filter during IBD) also for that.  */
    FeeFilterRounder btcs_rounder{CFeeRate{DEFAULT_MIN_RELAY_TX_FEE}, rng};
    BOOST_CHECK_EQUAL(btcs_rounder.round(MAX_MONEY), 9452957);
}
