...
def BestOverApproxInvLog2(mulof, maxd):
    ...
    for d in [1, 2, 9, 70, 131, 192, 445, 1588, 4319, 11369, 18419, 25469, 287209, 836158, 3057423, 10567111, 21950910, 35565709, 49180508, 161156323, 273132138, 385107953, 882191721]:
        kd = lcm(mulof, d)
        if kd <= maxd:
            n = ceil(kd / log(2))
            dis = F((n / kd) - 1 / log(2))
