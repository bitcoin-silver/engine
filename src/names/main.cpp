                      ...
                      TxValidationState& state, unsigned flags)
{
  ...
  /* Check that no name inputs/outputs are present for a non-Bitcoinsilver tx.
     If that's the case, all is fine.  For a Bitcoinsilver tx instead, there
  ...
  /* Reject "greedy names".  */
  const Consensus::Params& params = Params ().GetConsensus ();
  if (tx.vout[nameOut].nValue < params.rules->MinBitcoinSilverAmount(nHeight))
    return state.Invalid (TxValidationResult::TX_CONSENSUS,
                          "tx-name-greedy",
                      ...
                      CCoinsViewCache& view, CBlockUndo& undo)
{
  ...
  /* This check must be done *after* the historic bug fixing above!  Some
     ...
     transactions *not* marked as Bitcoinsilver tx.  */
  if (!tx.IsBitcoinsilver ())
    return;
