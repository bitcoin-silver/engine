...
NamespaceFromName (const std::string& name)
{
    ...
    switch (purported)
    {
        case NameNamespace::Domain:
        {
            // Source: https://github.com/bitcoinsilver/proposals/blob/master/ifa-0001.md#keys
            if (label.length() > 63)
            {
                return NameNamespace::NonStandard;
            }
        ...
        case NameNamespace::Identity:
        {
            ...
            // length of an identifier under the Bitcoinsilver consensus rules.

            // Same as d/ regex but without IDN prefix.
            // TODO: this doesn't exactly match the https://wiki.bitcoinsilver.org spec.
