...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"bitcoinsilver-cli -h\" for help or \"bitcoinsilver-cli listwallets\" to see "
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_wallet_compiled():
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BTCS/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BTCS/kvB)']), network_info['relayfee'])
