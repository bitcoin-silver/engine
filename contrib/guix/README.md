# Bootstrappable Bitcoinsilver Core Builds

This directory contains the files necessary to perform bootstrappable Bitcoinsilver
Core builds.
...
To build Bitcoinsilver Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bitcoinsilver Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoinsilver-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcoinsilver/guix.sigs).
...
This is especially helpful if you build both Bitcoin Core and Bitcoinsilver Core,
since it will let Bitcoin and Bitcoinsilver share the caches and SDKs.
